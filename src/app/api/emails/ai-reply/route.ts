/**
 * email.reply — Domain-specific AI route
 *
...
import { aiReplyEmail } from '@/lib/ai-activate/engine';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await aiReplyEmail(payload);

    if (!result.success) {
      return NextResponse.json(
        { error: result.error || 'AI call failed', action: 'email.reply' },
        { status: 502 }
      );
    }
