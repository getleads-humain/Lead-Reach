  ...
  try {
    ...
    if (extractSchema && typeof extractSchema === 'object' && Object.keys(extractSchema).length > 0) {
      try {
        ...
        extracted = await callLLMForJSON<Record<string, unknown>>(
          `You are a data extraction assistant. Extract structured data from the provided web page content according to the given schema. Return ONLY a valid JSON object matching the schema. If a field cannot be found, set it to null. Always respond in English.`,
