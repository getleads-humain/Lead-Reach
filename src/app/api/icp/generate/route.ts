...
USER INSTRUCTION: ${promptInstruction}`,
    ...
    for (const icp of icpData.icps) {
      try {
        ...
        const savedICP = await db.iCPProfile.create({
          data: {
            name: icp.icpTitle,
            description: icp.segmentDescription,
            industries: JSON.stringify(analysis.industrySectors || []),
            companySizes: JSON.stringify([]),
            locations: JSON.stringify([]),
            revenueRange: icp.ticketSize === 'high' ? '>$1M' : icp.ticketSize === 'mid' ? '$100K-$1M' : '<$100K',
            requiredTech: JSON.stringify([]),
            preferredTech: JSON.stringify([]),
            techSophisticationLevel: 'medium',
            digitalMaturityScore: 50,
            values: JSON.stringify([]),
            challenges: JSON.stringify(icp.decisionTriggers ? [icp.decisionTriggers] : []),
            goals: JSON.stringify([]),
            cultureTypes: JSON.stringify([]),
            buyingSignals: JSON.stringify(icp.buyingJourney ? [icp.buyingJourney] : []),
            engagementPatterns: JSON.stringify([]),
            triggerEvents: JSON.stringify(icp.decisionTriggers ? [icp.decisionTriggers] : []),
            expansionSignals: JSON.stringify([]),
            complianceNeeds: JSON.stringify([]),
            budgetRange: icp.ticketSize === 'high' ? '>$1M' : icp.ticketSize === 'mid' ? '$100K-$1M' : '<$100K',
            decisionTimeline: null,
            priceSensitivity: 'medium',
            lifetimeValuePotential: icp.ticketSize === 'high' ? 'high' : icp.ticketSize === 'mid' ? 'medium' : 'low',
    ...
    return NextResponse.json({
      ...
      icps: savedICPs.map(icp => ({
        id: icp.id,
        icpTitle: (icp as Record<string, unknown>).name as string,
        segmentDescription: (icp as Record<string, unknown>).description as string,
        ticketSize: (icp as Record<string, unknown>).budgetRange as string,
        avendusProduct: '',
        kpiDashboard: '',
        decisionTriggers: '',
        buyingJourney: '',
