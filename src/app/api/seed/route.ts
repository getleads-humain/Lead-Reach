...
export async function POST() {
  try {
    ...
    const agentTasks = [
      ...
      { agentName: 'data-enrichment', taskType: 'enrich', status: 'running', priority: 8, progress: 72, campaignId: sgCampaign.id, input: JSON.stringify({ leads: 10 }), startedAt: new Date(Date.now() - 5300000) },
      { agentName: 'prospect-discovery', taskType: 'search', status: 'running', priority: 9, progress: 45, campaignId: ldnCampaign.id, input: JSON.stringify({ query: 'marketing agencies london', industry: 'Marketing', location: 'London' }), startedAt: new Date(Date.now() - 2000000) },
