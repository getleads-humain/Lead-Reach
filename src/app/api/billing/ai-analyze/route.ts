/**
 * billing.analyze — Domain-specific AI route
 *
...
import { aiAnalyzeBillingUsage } from '@/lib/ai-activate/engine';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await aiAnalyzeBillingUsage(payload);

    if (!result.success) {
      return NextResponse.json(
        { error: result.error || 'AI call failed', action: 'billing.analyze' },
        { status: 502 }
      );
    }

    return NextResponse.json({
      action: 'billing.analyze',
      result: result.data,
      modelUsed: result.modelUsed,
    });
  } catch (error) {
    console.error('[billing.analyze] Error:', error);
    return NextResponse.json(
