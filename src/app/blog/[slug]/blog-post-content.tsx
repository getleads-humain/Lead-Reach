...
import type { BlogPost, BlogPostMeta, BlogPostSection } from '@/lib/blog-metadata';

interface BlogPostContentProps {
  post: BlogPost;
  relatedPosts: BlogPostMeta[];
  otherPosts: BlogPostMeta[];
...
export function BlogPostContent({ post, relatedPosts, otherPosts, categoryColors }: BlogPostContentProps) {
  // Newsletter state
  ...
  const wordCount = post.sections.reduce((acc, s) => {
    if (s.type === 'paragraph' || s.type === 'h2' || s.type === 'h3' || s.type === 'quote' || s.type === 'callout') {
      return acc + (s.content?.split(/\s+/).length || 0);
    }
    ...
    <MarketingLayout>
      ...
      <section className="relative pt-8 pb-12 border-b border-border/20">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          ...
          {/* Category Badge */}
          <Badge variant="outline" className={`mb-4 ${categoryColors[post.category] || ''}`}>
            {post.category}
          </Badge>
                ...
                <Link key={related.id} href={`/blog/${related.slug}`}>
                  <Card className="card-premium border-border/30 bg-card/50 group cursor-pointer h-full">
                    ...
                    <CardContent className="p-5">
                      <Badge variant="outline" className={`text-[10px] mb-2 ${categoryColors[related.category] || ''}`}>
                        {related.category}
                      </Badge>
